        ...
        private void ToolBarFun_ItemClick(object sender, Feng.Forms.Controls.ToolBarItem item)
        {
            try
            {
                ...
                if (item.ID == "RUN")
                {
                    ...
                    try
                    {
                        error = ScriptBuilder.Debug(this.dataExcel1.EditView, this.dataExcel1.EditView.FocusedCell, script);
                    }
                    catch (Exception ex)
                    {
                        SyntacticException e = ex as SyntacticException;
                        if (e != null)
                        {
                            if (e.Token != null)
                            {
                                selectcodestart = e.Token.StartIndex;
                            }
                        }
                        error = ex.Message;
