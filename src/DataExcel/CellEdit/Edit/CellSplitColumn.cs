...
{
    [Serializable]
    public class CellSplitColumn : CellLabel
    {
        public CellSplitColumn(DataExcel grid)
            : base(grid)
        {
        ...
        public override bool OnMouseMove(object sender, MouseEventArgs e, EventViewArgs ve)
        {
            ...
            if (cell != null)
            {
                ...
                if (!move)
                {
                    if (cell.Rect.Contains(viewloaction))
                    {
                        cell.Grid.BeginSetCursor(Cursors.VSplit);
                        return true;
                    }
                }
                else
        ...
        {
            CellSplitColumn celledit = new CellSplitColumn(grid);
            return celledit;
        }
    }
