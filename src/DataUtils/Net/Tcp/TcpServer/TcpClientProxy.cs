...
{
    public class TcpClientProxy : TcpClientProxyBase
    {

        public TcpClientProxy(Socket socket, NetServer server)
        {
        ...
        public override void OnDataReceive(ReciveEventArgs e, NetPacket ph)
        {
            base.OnDataReceive(e, ph);
        }

        public override void OnDataReceive(ReciveEventArgs e)
        {
            NetPacket ph = null;
            try
            {
                if (e.Data.Length < 1)
                    return;
                this.Server.OnBeforeDataReceive(e);
                if (e.Cancel)
                {
                    return;
                }
                ph = NetPacket.Get(e.Data);
                OnDataReceive(e, ph);
            ...
            catch (Exception ex)
            {
                Feng.Utils.BugReport.Log(ex);
                Utils.TraceHelper.WriteTrace("DataUtils", "GridViewCell", "OnDraw", ex);
                NetErrorTool.RespondError(e.ClientProxy, ph, ex);
            }
        }
        ...
        public override NetResult SendToRemote(NetPacket ph)
        {
            ...
            try
            {
                ...
                lock (SendLock)
                {
                    SocketError se = SocketError.IsConnected;
                    int len = 0;
                    while (len < data.Length)
                    {
                        int sendlen = this.Client.Send(data, len, data.Length - len, SocketFlags.None, out se);
                        len = len + sendlen;
                        if (se != SocketError.Success)
                        {
                            this.Close();
                            return NetResult.ErrorResult("Client Is Close", NetResult.CODE_USER);
                        }
                    }
        ...
        public override void OnGetReceiveData(object state)
        {
