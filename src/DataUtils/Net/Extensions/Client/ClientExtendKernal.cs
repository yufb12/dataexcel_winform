...
{
    public class ClientExtendKernal : ClientExtendBase, IDisposable
    {
        public ClientExtendKernal()
        {
            ...
            if (ph.PacketMainCommand == PacketMainCmd.System)
            {
                switch (ph.PacketSubcommand)
                {
                    ...
                    case PacketSystemSubCmd.Attach:
                        OnDoDomAttach(ph);
                        break;
                    case PacketSystemSubCmd.DoCommand: 
                        break;
                    default:
                        break;
                }
        ...
        public virtual byte[] OnDoCommand(string category, string command, string contents, byte[] data)
        {
            NetPacket ph = new NetPacket(
PacketMainCmd.System,
PacketSystemSubCmd.DoCommand);
            ph.PacketMode = PacketMode.Send;
            ph.BeginWriter();
            ph.Writer.Write(1, category);
            ph.Writer.Write(2, command);
            ph.Writer.Write(3, contents);
            ph.Writer.Write(4, data);
            ph.EndWriter();
            ...
            if (result.Success)
            {
                ph = result.Packet;
                ph.BeginRead();
                bool res = ph.Reader.ReadBoolean();
                if (res)
                {
                    data = ph.Reader.ReadBytes();
                    ph.EndRead();
                    return data;
                }
                ph.EndRead();
            ...
            {
                throw new Feng.Utils.AppException(result.Message, result.Code);
            }
            return null;
        }
        ...
        {
            string mac = Feng.App.Systeminfo.GetFirstEthernetMacAddress();
            NetResult result = RegeditSession(user, password,  System.Text.Encoding .UTF8.GetBytes(mac));
            ...
            using (Feng.IO.BufferWriter bw = new Feng.IO.BufferWriter())
            {
                ...
                if (fengresult.Success)
                {
                    ...
                    if (session > 1)
                    {
                        ...
                        this.Client.SessionKey = Feng.Utils.ConvertHelper.IntTo36(session);
                        this.Client.ID = id;
