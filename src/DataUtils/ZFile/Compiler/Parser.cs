...
namespace Feng.Complier
{
    public class Parser
    {
        ...
        public Parser(Lexer l) { lex = l; move(); }
        ...
        Stmt stmt()
        {
            ...
            switch (look.tag)
            {
                ...
                case Tag.IF:
                    match(Tag.IF); match('('); x = Abool(); match(')');
                    s1 = stmt();
                ...
                case Tag.DO:
                    ...
                    match(Tag.DO);
                    s1 = stmt();
                    match(Tag.WHILE); match('('); x = Abool(); match(')'); match(';');
                    donode.init(s1, x);
                    Stmt.Enclosing = savedStmt;  // reset Stmt.Enclosing
                    return donode;
        ...
        Stmt assign()
        {
            ...
            if (look.tag == '=')
            {       // S -> id = E ;
                move(); stmt = new Set(id, Abool());
            }
            else
            {                        // S -> L = E ;
                Access x = offset(id);
                match('='); stmt = new SetElem(x, Abool());
            }
            match(';');
            return stmt;
        }
