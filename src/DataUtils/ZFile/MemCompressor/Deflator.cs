...
{  
    ...
    {
        public const string Author = "Andrey Dryazgov";
        public const string AuthorLink = "http://www.codeproject.com/Members/Andrey-Dryazgov";
        ...
        public static unsafe int Qbyte(int[] data)
        {
            ...
            fixed (int* t = &data[0])
            {
                ...
                {
                    pp++;
                    System.Diagnostics.Debug.WriteLine("*pp =" + pp[0].ToString());
                    pp[0] = 456;
                    System.Diagnostics.Debug.WriteLine("ttt=" + t[0].ToString());
                } return it;
            }
        ...
        {
            Compress(System.IO.File.ReadAllBytes(fromfile), AcedCompressionLevel.Fast, Tofile);
        }
        public void Compress(string fromfile, AcedCompressionLevel level, string Tofile)
        {
            Compress(System.IO.File.ReadAllBytes(fromfile), level, Tofile);
        }
        public void Compress(byte[] data, AcedCompressionLevel level, string Tofile)
        {
            System.IO.File.WriteAllBytes(Tofile, Compress(data, 0, data.Length, AcedCompressionLevel.Fast, 0, 0));
        }
        public void Compress(byte[] data, string Tofile)
        {
            Compress(data, AcedCompressionLevel.Fast, Tofile);
        }

        public byte[] Compress(byte[] data)
        {
            return Compress(data, 0, data.Length, AcedCompressionLevel.Fast, 0, 0);
        }
