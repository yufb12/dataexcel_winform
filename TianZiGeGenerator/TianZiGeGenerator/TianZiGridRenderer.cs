...
namespace TianZiGeGenerator
{
    public class TianZiGridRenderer
    {
        ...
        private void DrawGridArea(Graphics g, PageLayout pageLayout, int pageIndex)
        {
            List<string> words = ParseWords(_config.InputText);
            if (words.Count == 0) return;

            int practiceRow = 1 + _config.PracticeRows;
            int startRow = pageIndex * pageLayout.Rows;
            int endRow = startRow + pageLayout.Rows;

            int gray = _config.TraceGrayLevel;
            if (gray < 0) gray = 0; if (gray > 255) gray = 255;
            Color tc = Color.FromArgb(gray, gray, gray);

            float cellTextSize = _config.CellSize * 0.72f * 2.835f;
            if (cellTextSize < 4f) cellTextSize = 4f;
            float cellPinYinTextSize = _config.CellSize * 0.2f * 2.835f;
            if (cellPinYinTextSize < 3f) cellPinYinTextSize = 3f;

            using (Font cf = CreateFont(_config.FontName, cellTextSize))
            using (Font pf = new Font("Times New Roman", cellPinYinTextSize))
            ...
            using (SolidBrush pb = new SolidBrush(_config.PinyinColor))
            {
                ...
                for (int row = startRow; row < endRow && row < words.Count * practiceRow; row++)
                {
                    int lr = row - startRow;
                    int wi = row / practiceRow;
                    int riw = row % practiceRow;
                    string word = words[wi];
                    float ry = pageLayout.GridStartY + lr * pageLayout.RowHeight;
                    float gy = ry + pageLayout.PinyinHeight;
                    DrawStrokeDemoRow(g, pageLayout, word, ry, gy, cf, pf, tb, sb, pb, cfmt, pfmt);
                }
            }
        }
