...
namespace MindMap.Layout
{
    ...
    /// 水平思维导图布局引擎
    /// 适用场景：标准思维导图、头脑风暴
    /// 布局特点：根节点在中心，子节点向左右两侧对称展开
    /// </summary>
    public class MindMapHorizontalLayout : ILayoutEngine
    {
        ...
        public void Layout(MindMapDocument document)
        {
            ...
            for (int i = 0; i < rightCount; i++)
            {
                ...
                LayoutChildrenHorizontal(child, HORIZONTAL_SPACING * 1.5f, true);
            }

            // 布局左侧节点（向左展开）
            for (int i = 0; i < leftCount; i++)
            {
                MindMapNode child = children[rightCount + i];
                float y = (i - leftCount / 2f) * VERTICAL_SPACING;
                child.Position = new PointF(-HORIZONTAL_SPACING, y);
                LayoutChildrenHorizontal(child, -HORIZONTAL_SPACING * 1.5f, false);
            }
        }
        ...
        /// 水平布局子节点
        /// </summary>
        private void LayoutChildrenHorizontal(MindMapNode parent, float baseX, bool toRight)
        {
            ...
            for (int i = 0; i < count; i++)
            {
                ...
                LayoutChildrenHorizontal(child, x + direction * HORIZONTAL_SPACING * 0.6f, toRight);
            }
        }
    }
}
